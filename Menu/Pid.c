...
void pidRun(void)
{
  ...
  //mustStoreCmd("M117 Cantidad %d",tool);

  if (tool < MAX_HEATER_PID_COUNT)
  {
    //mustStoreCmd("M117 %s S%d U1\n",(infoMachineSettings.firmwareType == FW_REPRAPFW) ? pidCmdRRF[tool] : pidCmdMarlin[tool], (int)pidHeaterTarget[tool]);
    mustStoreCmd("%s S%d U1\n", (infoMachineSettings.firmwareType == FW_REPRAPFW) ? pidCmdRRF[tool] : pidCmdMarlin[tool], (int)pidHeaterTarget[tool]);  // start PID autotune
